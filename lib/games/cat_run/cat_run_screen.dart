...
import 'cat_run_game.dart';
import 'components/play_buttons.dart';

/// Hosts [CatRunGame], the two play buttons and the shared home button.
///
...
///
/// Each button is [PlayButton.buttonSize] (140) drawn, but sits inside an
/// invisible box covering its whole bottom quarter of the screen. A
/// five-year-old mid-jump does not look at their thumb, so **anywhere in the
/// bottom-left ducks and anywhere in the bottom-right jumps** — the scope asks
/// for exactly this, and it is the difference between a game that answers and a
/// game that seems to ignore them (CLAUDE.md §3).
///
/// The home button is top-right, far from both, so it cannot be hit mid-jump.
class CatRunScreen extends StatefulWidget {
  const CatRunScreen({super.key});

  @override
  State<CatRunScreen> createState() => _CatRunScreenState();
}

class _CatRunScreenState extends State<CatRunScreen> {
  late final CatRunGame _game;

  /// The game, for the test that drives a press through the real widget tree.
  @visibleForTesting
  CatRunGame get gameForTest => _game;

  @override
  void initState() {
    super.initState();
    context.read<AudioController>().playSong(Songs.play);
    _game = CatRunGame(sounds: KidSounds(context.read<AudioController>()));
  }
      ...
      body: Stack(
        children: [
          ...
          _HitArea(
            ...
            kind: PlayButtonKind.crouch,
          ),
          _HitArea(
            alignment: Alignment.bottomRight,
            onPressed: _game.pressJump,
            kind: PlayButtonKind.paw,
          ),
...
class _HitArea extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Positioned(
      ...
      right: isLeft ? null : 0,
      bottom: 0,
      width: screen.width * PlayButton.hitAreaFraction,
      height: screen.height * PlayButton.hitAreaFraction,
        ...
        child: Stack(
          children: [
            Align(
              alignment: alignment,
              child: Padding(
                // Clear of the very corner, so a hand resting on the bezel
                // does not sit on the button, but well within the quarter.
                padding: const EdgeInsets.all(22),
                child: IgnorePointer(
                  // The quarter above handles the press; this is the picture.
                  // Without this, the small button would swallow the press and
                  // the generous area would only work outside it.
                  child: PlayButton(
                    kind: kind,
                    onPressed: onPressed,
